...
public class Dial
	...
	public void $(final HttpMethod method, final HttpServletRequest request, final HttpServletResponse response)
		...
		log.info(format("[%d] Dial %s(%s) for %s on %s(%s) cid: %s<%s>", id, number, contact, agent, site, effectiveQueue,
		                cidName, cidNumber));
		try {
			final String dial = format("SIP/%s", agent);

			asterisk.originateToExtensionAsync(dial, "from-internal", number, 1, 30000, new CallerId(cidName, cidNumber),
